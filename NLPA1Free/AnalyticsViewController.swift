...
class AnalyticsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let webString = String(format:"<div style='background-color:transparent'><div style='text-align:center'><canvas id='canvas' width='200' height='200'></canvas></div><br/><div style='text-align:center;color:#fff'><samp style='width:20px;height:20px;background-color:#454FDF;font-size:16px;border-radius:5px'>&nbsp;V:%2.1f%%&nbsp;</samp>&nbsp;<samp style='width:20px;height:20px;background-color:#F28107;font-size:16px;border-radius:5px'>&nbsp;A:%2.1f%%&nbsp;</samp>&nbsp;<samp style='width:20px;height:20px;background-color:#48990B;font-size:16px;border-radius:5px'>&nbsp;K:%2.1f%%&nbsp;</samp><div style='color:#000000;text-align:left'>%@</div><script type='text/javascript'>var myColor = ['#454FDF','#F28107','#48990B','#542437','#53777A'];var myData = [%2.4f,%2.4f,%2.4f,0,0];function getTotal(){var myTotal = 0;for (var j = 0; j < myData.length; j++) {myTotal += (typeof myData[j] == 'number') ? myData[j] : 0;}return myTotal;}function plotData() {var canvas;var ctx;var lastend = 0;var myTotal = getTotal();canvas = document.getElementById('canvas');ctx = canvas.getContext('2d');ctx.clearRect(0, 0, canvas.width, canvas.height);for (var i = 0; i < myData.length; i++) {ctx.fillStyle = myColor[i];ctx.beginPath();ctx.moveTo(100,100);ctx.arc(100,100,100,lastend,lastend+(Math.PI*2*(myData[i]/myTotal)),false);ctx.lineTo(100,100);ctx.fill();lastend += Math.PI*2*(myData[i]/myTotal);}}plotData();</script><br/><br/></div>",
                               a,b,c,
